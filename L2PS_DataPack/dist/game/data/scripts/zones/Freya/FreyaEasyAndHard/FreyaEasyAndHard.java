...
public class FreyaEasyAndHard extends Quest
{
	...
	protected L2PcInstance getRandomPlayer(FreyaWorld world)
	{
		...
		while (!exists)
		{
			L2PcInstance player = L2World.getInstance().getPlayer(world.getAllowed().get(getRandom(world.getAllowed().size())));
			if (player != null)
			{
				exists = true;
				return player;
	...
	private boolean checkConditions(L2PcInstance player)
	{
		...
		for (L2PcInstance partyMember : player.getParty().getCommandChannel().getMembers())
		{
			...
			if (_isHard)
			{
				QuestState st = player.getParty().getCommandChannel().getLeader().getQuestState("Q10286_ReunionWithSirra");
				if (!st.isCompleted())
				{
