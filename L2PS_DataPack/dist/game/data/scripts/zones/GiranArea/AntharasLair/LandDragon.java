 ...
 */
package zones.GiranArea.AntharasLair;
 ...
 * @author RobíkBobík
 */
public class LandDragon extends AbstractNpcAI
{
	// config
	...
	private static final int FWA_LIMITOFWEAK = 45;// 45 players for WEAK
	private static final int FWA_LIMITOFNORMAL = 63;// 63 players for NORMAL > for HARD
	// Ochranka count
	...
	private LandDragon(String name, String descr)
	{
		super(name, descr);
		registerMobs(ANTHARAS_WEAK_ID, ANTHARAS_NORMAL_ID, ANTHARAS_STRONG_ID, 29069, 29070, 29071, 29072, 29073, 29074, 29075, 29076);
		init();
	}
	...
	private void init()
	{
		...
		int status = GrandBossManager.getInstance().getBossStatus(ANTHARAS_WEAK_ID);
	...
	public void setAntharasSpawnTask()
	{
		if (_monsterSpawnTask == null)
		{
			synchronized (this)
			{
				if (_monsterSpawnTask == null)
				{
					GrandBossManager.getInstance().setBossStatus(ANTHARAS_WEAK_ID, WAITING);
		...
		public void run()
		{
			...
			switch (_taskId)
			{
				case 1: // Spawn.
					...
					else if ((_players.size() <= FWA_LIMITOFNORMAL) && (_players.size() > FWA_LIMITOFWEAK))
					{
						npcId = 29067; // normal
					}
					else
					...
					GrandBossManager.getInstance().setBossStatus(ANTHARAS_WEAK_ID, DORMANT);
	...
	{
		if ((npc.getNpcId() == 29066) || (npc.getNpcId() == 29067) || (npc.getNpcId() == 29068))
		{
			_LastAction = System.currentTimeMillis();
	...
	{
		new LandDragon(LandDragon.class.getSimpleName(), "zones/GiranArea/AntharasLair/");
	}
}