...
public class TeleportManager implements ScriptFile, ICommunityBoardHandler
{
	...
	public void onBypassCommand(Player player, String command)
	{
		...
		{
			ShowBoard.separateAndSend("<html><body><br><br><center>Function: " + command + " not implemented yet</center><br><br></body></html>", player);
		}
	}
	...
	private void goTp(Player player, int xTp, int yTp, int zTp, int priceTp)
	{
		...
		if (Config.BBS_TELEPORT_PEACEZONE_ONLY && !player.isInZonePeace())
		{
			player.sendMessage(player.isLangRus() ? "Community Teleport можно использовать только в мирной зоне" : "Community Teleport can be used only in a peaceful area");
			return;
		}
	...
	private void showTp(Player player)
	{
		...
		try
		{
			...
			while (rs.next())
			{
				...
				html.append("</td>");
				html.append("<td>");
				html.append("<button value=\"Remove\" action=\"bypass _bbsteleport;delete;" + tp.TpId + "\" width=100 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">");
				html.append("</td>");
				html.append("</tr>");
			}
			html.append("</table>");
