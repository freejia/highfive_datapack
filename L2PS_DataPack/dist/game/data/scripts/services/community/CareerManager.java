...
public class CareerManager implements ScriptFile, ICommunityBoardHandler
{
		...
		if (command.startsWith("_bbscareer;sub;"))
		{
			...
			switch (intVal)
			{
				...
				case 2: // Установка уже взятого саба (см case 5)
					html.append("<br>Switch the sub-class:<br>");
					
					final int baseClassId = activeChar.getBaseClassId();
					
					if (playerClassList.size() < 2)
					{
						html.append("You do not have a sub-class to change, but you can add it right now<br><a action=\"bypass _bbscareer;sub;1\">Add sub.</a>");
					}
					else
					{
						html.append("What sub-class you wish to use?<br>");
						
						if (baseClassId == activeChar.getActiveClassId())
						{
							html.append(HtmlUtils.htmlClassName(activeChar.getActiveClassId()) + " <font color=\"LEVEL\">(Base)</font><br><br>");
						}
						else
						{
							html.append("<a action=\"bypass _bbscareer;sub;5 " + baseClassId + "\">" + HtmlUtils.htmlClassName(activeChar.getActiveClassId()) + "</a> " + "<font color=\"LEVEL\">(Base)</font><br><br>");
						}
				...
				case 3: // Отмена сабкласса - список имеющихся (см case 6)
					html.append("<br>Cancel Sub-class: <br> Which of the existing subs you want to replace?<br>");
				...
				case 4: // Добавление сабкласса - обработка выбора из case 1
					...
					{
						activeChar.sendMessage(new CustomMessage("l2f.gameserver.model.instances.L2VillageMasterInstance.NoSubBeforeLevel", activeChar).addNumber(Config.ALT_GAME_LEVEL_TO_GET_SUBCLASS));
						allowAddition = false;
					}
					...
					if (allowAddition)
					{
						...
						html.append("<br><br>Sub-class <font color=\"LEVEL\">" + className + "</font> successfully added!");
						activeChar.sendPacket(SystemMsg.CONGRATULATIONS__YOUVE_COMPLETED_A_CLASS_TRANSFER); // Transfer to new class.
					}
					else
					{
						html.append("<br><br>You can not add a subclass of the moment.<br>To activate the subclass you must achieve <font color=F2C202>Level 75</font><br>");
					}
					break;
				case 5: // Смена саба на другой из уже взятых - обработка выбора из case 2
					...
					html.append("<br>Your active sub-class is now: <font color=\"LEVEL\">" + HtmlUtils.htmlClassName(activeChar.getActiveClassId()) + "</font>.");
				...
				case 6: // Отмена сабкласса - обработка выбора из case 3
					html.append("<br><br>Select a sub-class to change.<br>" + //
					"<font color=\"LEVEL\">Attention!</font> All professions and skills for this subwoofer will be removed.<br><br>");
				...
				case 7: // Отмена сабкласса - обработка выбора из case 6
					...
					if (activeChar.modifySubClass(classId, newClassId))
					{
						...
						html.append("<br>Your sub-class is changed to: <font color=\"LEVEL\">" + HtmlUtils.htmlClassName(activeChar.getActiveClassId()) + "</font>.");
	...
	private StringBuilder getSubClassesHtml(Player activeChar, boolean condition)
	{
		...
		if ((subsAvailable != null) && !subsAvailable.isEmpty() && condition)
		{
			...
			{
				html.append("<br><font color=F2C202>" + activeChar.getName() + "</font> You have the following operations on the sub-classes:<br><br>back to the city. Operations on the subwoofer are only available in");
			}
			else
			{
				html.append("<br><font color=F2C202>" + activeChar.getName() + "</font> You have the following operations on the sub-classes:<br>");
				html.append("<center><table width=600><tr>");
				html.append("<td><center><button value=\"Add\" action=\"bypass _bbscareer;sub;1\" width=150 height=20 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\"></center></td>");
				html.append("<td><center><button value=\"Change\" action=\"bypass _bbscareer;sub;2\" width=150 height=20 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\"></center></td>");
				html.append("<td><center><button value=\"Cancel\" action=\"bypass _bbscareer;sub;3\" width=150 height=20 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\"></center></td>");
				html.append("</tr></table></center>");
			}
		}
		else
