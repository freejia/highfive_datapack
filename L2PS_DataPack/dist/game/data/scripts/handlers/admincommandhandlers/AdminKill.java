...
public class AdminKill implements IAdminCommandHandler
{
	...
	private void kill(L2PcInstance activeChar, L2Character target)
	{
		...
		else if (Config.EASY_CHAMPION_ENABLE && target.isEasyChampion())
		{
			target.reduceCurrentHp((target.getMaxHp() * Config.EASY_CHAMPION_HP) + 1, activeChar, null);
		}
		else if (Config.HARD_CHAMPION_ENABLE && target.isHardChampion())
		{
			target.reduceCurrentHp((target.getMaxHp() * Config.HARD_CHAMPION_HP) + 1, activeChar, null);
		}
		else
		{
